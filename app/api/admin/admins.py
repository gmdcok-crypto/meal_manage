...
def list_admins(db: Session = Depends(get_db), _admin=Depends(get_current_admin)):
    """식당관리 목록 (사번, 이름, 인증 상태)."""
    result = db.execute(
...
def create_admin(
    ...
    existing = db.execute(select(CafeteriaAdmin).where(CafeteriaAdmin.emp_no == body.emp_no))
...
def delete_admin(admin_id: int, db: Session = Depends(get_db), _admin=Depends(get_current_admin)):
    """식당관리 삭제."""
    result = db.execute(select(CafeteriaAdmin).where(CafeteriaAdmin.id == admin_id))
    admin = result.scalar_one_or_none()
    if not admin:
...
def reset_admin_device(admin_id: int, db: Session = Depends(get_db), _admin=Depends(get_current_admin)):
